...
public class DefaultShardingAction implements ShardingAction {
    ...
    public Collection<SqlExecutionUnit> doSharding() {
        ...
        List<AlgorithmResult> algorithmResultList = new ArrayList<AlgorithmResult>(1);
        for (ShardingValue shardingValue : shardingValues) {
            algorithmResultList.add(algorithm.doAlgorithm(shardingValue));
        }
        for (AlgorithmResult algorithmResult : algorithmResultList) {
    ...
    private ShardingValue constructShardingValue(ShardingRule shardingRule, RouterCondition condition) {
        //所有的表规则(表以及列)
        List<ShardingKey> shardingKeyList = shardingRule.getShardingKey();
